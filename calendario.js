...
function renderSchedule() {
  ...
  document.getElementById("scheduleList").innerHTML = matches.length ? matches.map((match) => `<article class="schedule-match"><div><span class="schedule-match__league">${leagueName(match.league)}</span><span class="schedule-match__phase">${escapeHtml(match.phase)}</span></div><strong>${escapeHtml(match.playerOne)} <i>vs</i> ${escapeHtml(match.playerTwo)}</strong><time>${formatDate(match.date)}</time></article>`).join("") : '<p class="schedule-empty">No hay matches pendientes para esta liga.</p>';
}

function leagueName(league) { return { master: "Master", diamond: "Diamante", platinum: "Platino" }[league] || escapeHtml(league); }
